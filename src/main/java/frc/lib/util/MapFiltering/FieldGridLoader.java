...
public class FieldGridLoader {
  ...
  public static GridMap load(String fileName) {
    try {
      Path path =
          Filesystem.getDeployDirectory().toPath().resolve("MapFiltering").resolve(fileName);
      ...
      return new GridMap(data.grid, data.nodeSizeMeters, data.field_size.x, data.field_size.y);
