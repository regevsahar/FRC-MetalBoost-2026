...
*/
	...
	public static Command createPathDuringRuntime(Pose2d currentPose, Pose2d targetPose, PathConstraints constraints) {
		...
		PathPlannerPath path = new PathPlannerPath(bezierPoints, constraints, null,
		 new GoalEndState(0, targetPose.getRotation()));
		path.preventFlipping = true;
		return PathFollowingCommandsBuilder.followPath(path);
	}

	public static Command createPathDuringRuntime(Pose2d currentPose, Pose2d targetPose, PathConstraints constraints, boolean mirror) {
		List<Waypoint> bezierPoints = PathPlannerPath.waypointsFromPoses(currentPose, targetPose);
		PathPlannerPath path = new PathPlannerPath(bezierPoints, constraints, null,
		 new GoalEndState(0, targetPose.getRotation()));
	...
	public static Command GoToNearestBranch(Pose2d currentPose, PathConstraints constraints, boolean mirror){
        Reef closest = null;
	...
	public static boolean isRobotInPathfindingDeadband(Pose2d currentPose, Pose2d targetPose) {
		return ToleranceMath.isNear(targetPose.getTranslation(), currentPose.getTranslation(), 0.5); // TODO change to constant
	}
/*
